...
class com_virtuemart_allinoneInstallerScript {
	...
	public function postflight($type, $parent = null) {
        ...
        $this->installPlugin('VM - Pangalink, Swedbank', 'plugin', 'swedbank', 'vmpayment');
	...
	function com_install() {
	    ...
	    if(version_compare(JVERSION,'1.6.0','ge')) {
	       // Joomla! 1.6 code here
	       $app -> enqueueMessage('1.6');
	       $app -> enqueueMessage($vmInstall->postflight($method));
	    } else {
	    	// Joomla! 1.5 code here
	        $method = ($upgrade) ? 'update' : 'install';
	
	        //$vmInstall->$method();
	        $app -> enqueueMessage('1.5');
	        $app -> enqueueMessage($vmInstall->postflight($method));
	    }
	    return true;
	}
